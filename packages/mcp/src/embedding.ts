import { OpenAIEmbedding, VoyageAIEmbedding, GeminiEmbedding, OllamaEmbedding, InfinityEmbedding } from "@zilliz/claude-context-core";
import { ContextMcpConfig } from "./config.js";

// Helper function to create embedding instance based on provider
export function createEmbeddingInstance(config: ContextMcpConfig): OpenAIEmbedding | VoyageAIEmbedding | GeminiEmbedding | OllamaEmbedding | InfinityEmbedding  {
...
export function logEmbeddingProviderInfo(config: ContextMcpConfig, embedding: OpenAIEmbedding | VoyageAIEmbedding | GeminiEmbedding | OllamaEmbedding | InfinityEmbedding): void {
    ...
    switch (config.embeddingProvider) {
        ...
        case 'Ollama':
            ...
            break;
        case 'Infinity':
            console.log(`[EMBEDDING] Infinity configuration - URL: ${config.infinityUrl || 'http://localhost:7997'}, Model: ${config.embeddingModel}`);
            break;
    }
}
